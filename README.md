...
- Previous day consumption sensor (kWh) with `state_class: total`, a data-driven `last_reset` set to yesterday's local midnight (`previous_day_consumption_last_reset`), and two data-quality attributes:
  ...
  - If `entry_count` is unexpectedly low (e.g. 1), enable debug logging for `custom_components.eon_next` — the integration logs per-page result counts and pagination state for every consumption fetch, which identifies whether the issue is a missing date window, API pagination cap, or smart meter comms gap.
